...
struct OldPublishController {
    private let shell: any NnexShell
    private let picker: any NnexPicker
    private let fileSystem: any FileSystem
    private let gitHandler: any GitHandler
    private let service: any OldPublishService
...
// MARK: - Actions
extension OldPublishController {
    func publish(projectPath: String?, releaseNumber: ReleaseNumber?, noteSource: ReleaseNoteSource?, commitMessage: String?) throws {
        ...
        let buildConfig = makeConfig(testCommand: nil)

        _ = makePublishRequest(
            projectFolder: projectFolder,
            formulaInfo: formulaInfo,
            releaseInfo: releaseInfo,
            buildConfig: buildConfig,
            commitMessage: commitMessage
        ) // future NnexKit service input
...
private extension OldPublishController {
...
private extension OldPublishController {
    ...
    func makeReleasePlan(from releaseInfo: ReleaseInfo) -> ReleasePlan {
        let versionSpecifier = makeReleaseVersionSpecifier(from: releaseInfo.nextRelease)

        return ReleasePlan(previousVersion: releaseInfo.previous, next: versionSpecifier)
    }

    func makeReleaseVersionSpecifier(from releaseNumber: ReleaseNumber) -> ReleaseVersionSpecifier {
        switch releaseNumber {
        case .exact(let value):
            return .exact(value)
        case .increment(let component):
            return .increment(makeReleaseComponent(from: component))
        }
    }

    func makeReleaseComponent(from component: ReleaseNumber.Component) -> ReleaseComponent {
        switch component {
        case .major:
            return .major
        case .minor:
    ...
    func makeReleaseNotes(from source: ReleaseNoteSource) -> ReleaseNotes {
        switch source {
...
extension OldPublishController.ReleaseNumber.Component: DisplayablePickerItem {
    var displayName: String {
        return rawValue
    }
}

extension OldPublishController.NoteContentType: DisplayablePickerItem {
    var displayName: String {
        switch self {
        case .direct:
